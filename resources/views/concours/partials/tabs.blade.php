    ...
    <nav class="flex border-b border-gray-200">
        ...
        @if ($concours->type_ffe_sif && $concours->discipline === \App\Enums\Discipline::OPEN)
            <a href="{{ route('concours.statistiques.index', $concours) }}"
                class="px-6 py-3 text-sm font-medium border-b-2 {{ ($active ?? '') === 'statistiques' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                Statistiques
            </a>
            <a href="{{ route('concours.championnats.index', $concours) }}"
                class="px-6 py-3 text-sm font-medium border-b-2 {{ ($active ?? '') === 'championnats' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                Championnats
            </a>
        @endif
    </nav>
