...
<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg" x-data="commandeRetraitsApp({{ Js::from($commandes->map(fn($c) => [
                'id' => $c->id,
                ...
                <div x-show="editing" @keydown.escape.window="editing = null"
                    ...
                    <div class="bg-white rounded-lg shadow-xl max-w-lg w-full p-6" @click.stop>
                        <h3 class="text-lg font-semibold mb-4">Modifier la commande</h3>
    ...
    <script>
        function commandeRetraitsApp(initialCommandes, concoursId) {
            return {
                concoursId,
                ...
                async setQty(c, qty) {
                    ...
                    c.quantite_retiree = qty; // optimistic
                    try {
                        const res = await fetch(`/concours/${this.concoursId}/commande-retraits/${c.id}/set-quantite`, {
                            method: 'PATCH',
                            headers: {
                ...
                async saveEdit() {
                    ...
                    try {
                        const res = await fetch(`/concours/${this.concoursId}/commande-retraits/${c.id}`, {
                            method: 'PATCH',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                                'Accept': 'application/json',
