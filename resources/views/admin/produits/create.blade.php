    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('admin.produits.store') }}" class="space-y-6">
                    @csrf
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="prix_ttc" name="prix_ttc" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('prix_ttc')" required />
                            <x-input-error class="mt-2" :messages="$errors->get('prix_ttc')" />
                        </div>

                        <div>
                            <x-input-label for="tva" value="TVA (%)" />
                            <x-text-input id="tva" name="tva" type="number" step="0.01" min="0" max="100" class="mt-1 block w-full" :value="old('tva', '20.00')" required />
                            <x-input-error class="mt-2" :messages="$errors->get('tva')" />
                        </div>
                    </div>

                    <div class="flex items-center gap-4">
                        <x-primary-button>Créer le produit</x-primary-button>
